...
fire.past <- subset(fire.data, Species == "PAST")
fire.mcav <- subset(fire.data, Species == "MCAV")
...
KW_fvfm_mcav <- kruskal.test(Fv/Fm ~ Depth_group, data = fire.mcav)
KW_fvfm_mcav
DT_fvfm_mcav <- dunnTest(fire.mcav$"Fv/Fm" ~ fire.mcav$Depth_group, method = "bonferroni")
DT_fvfm_mcav
fvfm_mcav_let <- DT_fvfm_mcav$res
...
KW_sigma_mcav <- kruskal.test(Sigma ~ depth, data = fire.mcav)
KW_sigma_mcav
